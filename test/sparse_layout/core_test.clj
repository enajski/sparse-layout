(ns sparse-layout.core-test
  (:require [clojure.test :refer [deftest is testing]]
            [sparse-layout.core :as sparse :refer
             [block block-view col-blocks col-block-views col-id defsparse row-blocks
              row-block-views row-id]]))

(def ^Class double-array-type (Class/forName "[D"))

(defn- materialize-value [value] (if (instance? double-array-type value) (vec (seq value)) value))

(defn- materialize-blocks
  [blocks]
  (mapv (fn [[key value]]
          [key (materialize-value value)])
        blocks))

(defn- materialize-view [view] (sparse/block-view->vec view))

(defn- materialize-view-blocks
  [blocks]
  (mapv (fn [[key view]]
...
(defsparse
  variable-features
  {:row-key [:entity] :cols-path [:vals] :payload {:kind :var-double-block} :indices #{:csr :csc}})

(deftest compiles-fixed-double-block-layout
  (let [ds (entity-features-compile [{:entity 1
                                      :vals (array-map :f1 [1.0 2.0 3.0] :f2 [4.0 5.0 6.0])}
    ...
    (testing "generated accessors use the compiled arrays"
      (is (= [1.0 2.0 3.0] (materialize-value (entity-features-block ds 1 :f1))))
      (is (= [4.0 5.0 6.0] (materialize-value (entity-features-block ds 1 :f2))))
    ...
    (testing "CSR and CSC traversals return sparse blocks"
      ...
      (is (= [[1 [4.0 5.0 6.0]] [2 [7.0 8.0 9.0]]]
             (materialize-blocks (entity-features-col ds :f2)))))
    (testing "the shared protocol works at API boundaries"
      (is (= 0 (row-id ds 1)))
      (is (= 1 (col-id ds :f2)))
      (is (= [7.0 8.0 9.0] (materialize-value (block ds 2 :f2))))
      (is (= [[:f1 [1.0 2.0 3.0]] [:f2 [4.0 5.0 6.0]]] (materialize-blocks (row-blocks ds 1))))
      (is (= [[1 [4.0 5.0 6.0]] [2 [7.0 8.0 9.0]]] (materialize-blocks (col-blocks ds :f2)))))))

(deftest duplicate-policy-sum-coalesces-scalar-values
  (let [ds (scalar-features-compile [{:entity :a :vals (array-map :x 1.25)}
                                     {:entity :a :vals (array-map :x 2.75)}
                                     {:entity :a :vals (array-map :y 10.0)}])]
    (is (= 4.0 (scalar-features-block ds :a :x)))
    (is (= [[:x 4.0] [:y 10.0]] (scalar-features-row ds :a)))))

(deftest duplicate-policy-error-rejects-duplicates
  (is (thrown-with-msg? clojure.lang.ExceptionInfo
                        #"Duplicate sparse coordinate"
                        (duplicate-error-features-compile [{:entity :a :vals (array-map :x 1.0)}
                                                           {:entity :a
                                                            :vals (array-map :x 2.0)}]))))

(deftest duplicate-policy-merge-uses-merge-fn
  (let [ds (object-merge-features-compile [{:entity 1 :vals (array-map :meta {:a 1})}
                                           {:entity 1 :vals (array-map :meta {:b 2})}])]
    (is (= {:a 1 :b 2} (object-merge-features-block ds 1 :meta)))))

(deftest supports-variable-double-blocks
  (let [ds (variable-features-compile [{:entity 1 :vals (array-map :xs [1.0 2.0] :ys [3.0])}
                                       {:entity 2
                                        :vals (array-map :xs (double-array [4.0 5.0 6.0]))}])]
    (is (= [1.0 2.0] (materialize-value (variable-features-block ds 1 :xs))))
    (is (= [[1 [1.0 2.0]] [2 [4.0 5.0 6.0]]] (materialize-blocks (variable-features-col ds :xs))))))

(deftest exposes-zero-copy-block-views
  (let [ds
        (entity-features-compile [{:entity 1 :vals (array-map :f1 [1.0 2.0 3.0] :f2 [4.0 5.0 6.0])}
                                  {:entity 2 :vals (array-map :f2 [7.0 8.0 9.0])}])

        view
        (entity-features-block-view ds 1 :f2)]
    ...
    (is (= [7.0 8.0 9.0] (materialize-view (block-view ds 2 :f2))))
    (is (= [[:f1 [1.0 2.0 3.0]] [:f2 [4.0 5.0 6.0]]]
...
(deftest exposes-zero-copy-variable-block-views
  (let [ds
        (variable-features-compile [{:entity 1 :vals (array-map :xs [1.0 2.0] :ys [3.0])}
                                    {:entity 2 :vals (array-map :xs (double-array [4.0 5.0 6.0]))}])

        view
        (variable-features-block-view ds 2 :xs)]

    (is (= [4.0 5.0 6.0] (materialize-view view)))
    (is (= [[1 [1.0 2.0]] [2 [4.0 5.0 6.0]]]
...
(defn- csr-cols-strictly-increasing?
  [frozen]
  (let [^ints ptrs
        (:csr-row-ptrs frozen)
...
(defn- csc-rows-strictly-increasing?
  [frozen]
  (let [^ints ptrs
        (:csc-col-ptrs frozen)
...
(def ^:private base-layout {:row-key [:entity] :cols-path [:vals] :indices #{:csr :csc}})

(def ^:private comparison-cases
  [{:label :double-last
    :layout (assoc base-layout :payload :double)
    :entries [[3 :b 1.0] [1 :a 2.0] [3 :a 3.0] [1 :a 4.0] [2 :c 5.0] [3 :b 6.0] [1 :b 7.0]
              [2 :a 8.0]]}
   {:label :double-sum
    :layout (assoc base-layout
              :payload :double
              :duplicate-policy :sum)
    :entries [[3 :b 1.0] [1 :a 2.0] [3 :a 3.0] [1 :a 4.0] [2 :c 5.0] [3 :b 6.0] [1 :b 7.0]
              [2 :a 8.0]]}
   {:label :fixed-block-last
    ...
    :entries [[3 :b [1.0 2.0]] [1 :a [3.0 4.0]] [3 :a [5.0 6.0]] [1 :a [7.0 8.0]] [2 :c [9.0 10.0]]
              [1 :b [11.0 12.0]]]}
   {:label :var-block-last
    :layout (assoc base-layout :payload {:kind :var-double-block})
    :entries [[2 :x [1.0]] [1 :y [2.0 3.0]] [2 :x [4.0 5.0 6.0]] [1 :z [7.0]] [3 :y [8.0 9.0]]]}
   {:label :object-merge
    :layout (assoc base-layout
              :payload :object
              :duplicate-policy :merge
  ...
  (doseq [{:keys [label layout entries]} comparison-cases]
    (testing (str label ": freeze preserves lookup ordering invariants")
      (let [frozen (sparse/freeze-builder! (ingest-entries layout entries))]
        ...
        (is (csc-rows-strictly-increasing? frozen) "rows are sorted within each CSC column")))))

(deftest retain-coo-is-configurable-at-freeze-time
  (let [rows
        [{:entity 1 :vals (array-map :f1 [1.0 2.0 3.0] :f2 [4.0 5.0 6.0])}
         {:entity 2 :vals (array-map :f2 [7.0 8.0 9.0])}]

        no-coo-builder
        (sparse/make-builder {:row-key [:entity]
                              :cols-path [:vals]
                              :payload {:kind :fixed-double-block :dim 3}
                              :indices #{:csr :csc}
                              :retain-coo? false})
    ...
    (let [no-coo
          (sparse/freeze-builder! no-coo-builder)

          keep-coo
          (sparse/freeze-builder! keep-coo-builder)]
      ...
      (is (= [7.0 8.0 9.0]
             (materialize-value (sparse/block-by-ids 1
                                                     1
                                                     (:csr-row-ptrs no-coo)
                                                     (:csr-col-ids no-coo)
                                                     (:csc-col-ptrs no-coo)
