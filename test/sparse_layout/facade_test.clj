...
(deftest dataset-and-row-facades-behave-like-read-maps
  (let [ds
        (fixtures/entity-features-compile [{:entity 1
    ...
    (testing "nested row lookup"
      ...
      (is (= [4.0 5.0 6.0] (materialize-value (get row :f2))))
      (is (= [4.0 5.0 6.0] (materialize-value (get-in m [1 :f2]))))
...
(deftest column-facade-is-row-keyed
  (let [ds
        ...
        col
        (facade/col-map ds :f2)]

    (is (= 2 (count col)))
    (is (contains? col 1))
    (is (not (contains? col 3)))
    (is (= [7.0 8.0 9.0] (materialize-value (get col 2))))
...
(deftest view-facades-return-zero-copy-block-views
  (let [ds
        ...
        m
        (facade/as-view-map ds)

        row
        (facade/row-view-map ds 1)

        col
        (facade/col-view-map ds :f2)]

    (is (= [4.0 5.0 6.0] (materialize-view (get-in m [1 :f2]))))
    (is (= [1.0 2.0 3.0] (materialize-view (get row :f1))))
    (is (= [7.0 8.0 9.0] (materialize-view (get col 2))))))

(deftest scalar-view-facades-reject-block-views
  (let [ds
        (fixtures/scalar-features-compile [{:entity :a :vals (array-map :x 1.25)}])
