...
(defn- scalar-row-key [row-id]
  row-id)

(defn- scalar-col-key [col-id]
  (keyword (str "c" col-id)))

(defn- compound-row-key [row-id]
  {:tenant (str "tenant-" (mod row-id 17))
   :region (nth [:na :eu :apac :latam] (mod row-id 4))
   :bucket (quot row-id 1024)
   :entity row-id})

(defn- compound-col-key [col-id]
...
(defn row-key-fn [{:keys [key-shape]}]
  (case key-shape
    :scalar scalar-row-key
    :compound-map compound-row-key))

(defn col-key-fn [{:keys [key-shape]}]
  (case key-shape
    :scalar scalar-col-key
    :compound-map compound-col-key))

(defn equivalent-query-key [key]
  (if (map? key)
    (into {} key)
    key))

(defn row-col-ids [row-id col-count nnz-per-row]
  (mapv #(mod (+ (* row-id 31) (* % 17)) col-count)
        (range nnz-per-row)))

(defn payload [row-id col-id]
  (mapv (fn [i]
          (double (+ (* row-id 0.001)
                     (* col-id 0.01)
                     i)))
...
(defn make-entries
  ...
  (let [row-key (row-key-fn config)
        col-key (col-key-fn config)
        rows (mapv row-key (range row-count))
        cols (mapv col-key (range col-count))]
    (into []
          (mapcat (fn [row-id]
                    (map (fn [col-id]
                           [(nth rows row-id)
                            (nth cols col-id)
                            (payload row-id col-id)])
...
(defn entries->nested-row-index [entries]
  (reduce (fn [rows [row col payload]]
            (update rows row assoc col payload))
          {}
          entries))

(defn entries->nested-dual-index [entries]
  (reduce (fn [{:keys [rows cols]} [row col payload]]
            {:rows (update rows row assoc col payload)
             :cols (update cols col assoc row payload)})
          {:rows {}
           :cols {}}
          entries))
...
(defn sparse-direct-point-value ^double [ds row col]
  (sparse/block-view-value (bench-features-block-view ds row col) 0))

(defn sparse-map-point-value ^double [sparse-map row col]
...
(defn sparse-row-map-sum ^double [ds row]
  (reduce-kv (fn [sum _ view]
               (+ (double sum)
                  (sparse/block-view-value view 0)))
             0.0
             (facade/row-view-map ds row)))

(defn nested-row-map-sum ^double [nested-rows row]
...
(defn sparse-col-map-sum ^double [ds col]
  (reduce-kv (fn [sum _ view]
               (+ (double sum)
                  (sparse/block-view-value view 0)))
             0.0
...
(defn nested-dual-col-map-sum ^double [nested-dual col]
  (reduce-kv (fn [sum _ payload]
               (+ (double sum)
                  (first-payload-value payload)))
             0.0
...
(defn nested-row-only-col-sum ^double [nested-rows col]
...
(defn prepare [{:keys [row-count col-count nnz-per-row] :as config}]
  (let [entries (make-entries config)
        sparse (entries->sparse entries)
        ...
        col-key ((col-key-fn config) hot-col-id)
        scan-col ((col-key-fn config) (quot col-count 3))]
    {:config config
     :entries entries
     :sparse sparse
...
(defn nnz [{:keys [row-count nnz-per-row]}]
  (* row-count nnz-per-row))

(defn print-config [{:keys [label key-shape scale row-count col-count nnz-per-row] :as config}]
  (println)
  (println "===" label "===")
  (println "Config:" {:key-shape key-shape
                      :scale scale
                      :rows row-count
                      :cols col-count
...
(defn standard-configs []
  (filterv :standard? benchmark-configs))

(defn- selected-scales [args]
  (cond
    (some #{"all" "large"} args) #{:small :medium :large}
    (some #{"large-only"} args) #{:large}
    (some #{"medium"} args) #{:medium}
    (some #{"small"} args) #{:small}
    :else #{:small}))

(defn- selected-key-shapes [args]
  (cond
    (some #{"scalar"} args) #{:scalar}
    (some #{"compound"} args) #{:compound-map}
    :else #{:scalar :compound-map}))

(defn selected-configs [args]
  (let [scales (selected-scales args)
        key-shapes (selected-key-shapes args)]
    (filterv #(and (contains? scales (:scale %))
                   (contains? key-shapes (:key-shape %)))
             benchmark-configs)))
