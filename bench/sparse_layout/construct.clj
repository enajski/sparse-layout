...
(defn- format-us [ns] (format "%.1f µs" (/ (double ns) 1000.0)))

(defn- run-config
  [config reps]
  (data/print-config config)
  (let [entries
        (data/make-entries config)

        builder
        (ingest-builder entries)

        freeze!
        #(sparse/freeze-builder! builder)

        elapsed-ns
        (mean-ns freeze! reps)

        allocated-bytes
        (measure-allocation freeze! reps)]

    (println)
    (println (format "%-26s %16s" "metric" "freeze"))
    (println (format "%-26s %16s" "freeze mean time" (format-us elapsed-ns)))
    (println (format "%-26s %16s" "freeze alloc / call" (format-bytes allocated-bytes)))
    (println
      (format "%-26s %16.1f" "freeze alloc B/nnz" (/ (double allocated-bytes) (data/nnz config))))))
...
(defn -main [& args] (run (data/selected-configs args)))
