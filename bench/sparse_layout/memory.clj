...
(defn- measure-bytes [value] (long (mm/measure value :bytes true)))

(defn- print-memory-row
  [config label bytes]
  (let [entries (data/nnz config)]
    (println
      (format "%-52s %14s %12.1f B/nnz" label (format-bytes bytes) (/ (double bytes) entries)))))
...
(defn- memory-config
  ...
  (let [{:keys [entries sparse nested-rows nested-dual nested-vectorz]} (data/prepare config)]
    ...
    (print-memory-row config
                      "nested row-only index (persistent vector)"
                      (measure-bytes nested-rows))
...
(defn -main [& args] (memory-report (data/selected-configs args)))
