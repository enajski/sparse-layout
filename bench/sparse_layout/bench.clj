...
(defn- run-config [quick? config]
  ...
  (let [{:keys [entries sparse sparse-map nested-rows nested-dual row-key col-key scan-col]} (data/prepare config)]
    ...
    (run-case quick? "point lookup, map API: nested row index"
    ...
    (run-case quick? "row scan, row index: nested row map reduce-kv"
    ...
    (run-case quick? "construction from entries: nested row index"
...
(defn- smoke-config [config]
  (data/print-config config)
  (let [{:keys [sparse sparse-map nested-rows nested-dual row-key col-key scan-col]} (data/prepare config)
