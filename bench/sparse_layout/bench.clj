...
(defn- run-case
  ...
  (println)
  (println label)
  (if quick? (criterium/quick-bench (f)) (criterium/bench (f))))

(defn- run-config
  [quick? config]
  (data/print-config config)
  (let [{:keys [entries sparse sparse-map nested-rows nested-dual nested-vectorz row-key col-key
                scan-col]}
        (data/prepare config)]
    (println "Point lookup row/col:" [row-key col-key])
    (println "Column scan col:" scan-col)
    (run-case quick?
              "point lookup, generated sparse API: block-view first value"
              #(data/sparse-direct-point-value sparse row-key col-key))
    (run-case quick?
              "point lookup, map API: sparse facade"
              #(data/sparse-map-point-value sparse-map row-key col-key))
    (run-case quick?
              "point lookup, map API: nested row index (persistent vector)"
              #(data/nested-row-point-value nested-rows row-key col-key))
    (run-case quick?
              "point lookup, map API: nested row index (vectorz)"
              #(data/nested-vectorz-point-value nested-vectorz row-key col-key))
    (run-case quick?
              "row scan, row index: sparse facade reduce-kv"
              #(data/sparse-row-map-sum sparse row-key))
    (run-case quick?
              "row scan, row index: nested row map reduce-kv (persistent vector)"
              #(data/nested-row-map-sum nested-rows row-key))
    (run-case quick?
              "row scan, row index: nested row map reduce-kv (vectorz)"
              #(data/nested-vectorz-row-map-sum nested-vectorz row-key))
    (run-case quick?
              "column scan, column index: sparse facade reduce-kv"
              #(data/sparse-col-map-sum sparse scan-col))
    (run-case quick?
              "column scan, column index: nested dual index reduce-kv"
              #(data/nested-dual-col-map-sum nested-dual scan-col))
    (run-case quick?
              "column scan, row-only nested index: full row scan"
              #(data/nested-row-only-col-sum nested-rows scan-col))
    (run-case quick?
              "construction from entries: sparse COO ingest + CSR/CSC freeze"
              #(data/entries->sparse entries))
    (run-case quick?
              "construction from entries: nested row index (persistent vector)"
              #(data/entries->nested-row-index entries))
    (run-case quick?
              "construction from entries: nested row index (vectorz)"
              #(data/entries->nested-row-vectorz-index entries))
    (run-case quick?
              "construction from entries: nested dual row+column index"
...
(defn run-benchmarks
  ...
  ([configs quick?]
   ...
   (println
     "Modes: default=standard quick set, large/all=all increments, full=Criterium full bench, smoke=single execution")
...
(defn- smoke-config
  [config]
  (data/print-config config)
  (let [{:keys [sparse sparse-map nested-rows nested-dual nested-vectorz row-key col-key scan-col]}
        (data/prepare config)
    ...
    (println {:point point :row-sum row-sum :col-sum col-sum :row-only-col-sum row-only-col-sum})))
