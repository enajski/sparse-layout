...
(def ^:private ^"[B" artifact-magic
  (byte-array (concat (seq (.getBytes artifact-magic-prefix StandardCharsets/US_ASCII))
...
(def ^:private type-ids {:int32 1 :float64 2 :int64 3 :bytes 4 :utf8-json 5})
...
(def ^:private key-tags
  {:nil 0 :false 1 :true 2 :int64 3 :float64 4 :string 5 :keyword 6 :vector 7})
...
(defn- put-double-le!
  [^ByteArrayOutputStream out ^double value]
  (put-long-le! out (Double/doubleToRawLongBits value)))

(defn- utf8-bytes ^bytes [value] (.getBytes ^String value "UTF-8"))

(defn- put-bytes! [^ByteArrayOutputStream out ^bytes bytes] (.write out bytes 0 (alength bytes)))
...
(defn- encode-key!
  [^ByteArrayOutputStream out value]
  (cond
    (nil? value) (.write out (int (:nil key-tags)))
    (false? value) (.write out (int (:false key-tags)))
    (true? value) (.write out (int (:true key-tags)))
    (integer? value) (do (.write out (int (:int64 key-tags))) (put-long-le! out (long value)))
    (float? value) (do (.write out (int (:float64 key-tags))) (put-double-le! out (double value)))
    (string? value) (do (.write out (int (:string key-tags))) (encode-string! out value))
    (keyword? value) (do (.write out (int (:keyword key-tags)))
                         (if-let [ns (namespace value)]
                           (encode-string! out ns)
                           (put-int-le! out -1))
                         (encode-string! out (name value)))
    (vector? value) (do (.write out (int (:vector key-tags)))
    ...
    (throw
      (ex-info
        "CSR artifact keys must be nil, booleans, integers, doubles, strings, keywords, or vectors."
        {:key value
         :class (some-> value
                        class
                        str)}))))

(defn- encode-key-section
  [keys]
  (let [offsets
        (long-array (inc (count keys)))
    ...
    {:offsets offsets :bytes (.toByteArray out)}))

(defn- buffer-order! ^ByteBuffer [^ByteBuffer buffer] (.order buffer ByteOrder/LITTLE_ENDIAN))

(defn- bb-get-int
  ^long [^ByteBuffer buffer ^long offset]
...
(defn- decode-string-at
  [^ByteBuffer buffer ^long offset ^long limit]
  (let [length
        (bb-get-int buffer offset)
    ...
    (let [end (+ next length)]
      (when (> end limit)
        (throw (ex-info "String key extends past key boundary."
                        {:offset offset :end end :limit limit})))
...
(defn- decode-key-at
  [^ByteBuffer buffer ^long offset ^long limit]
  (when (>= offset limit)
    (throw (ex-info "Missing CSR artifact key tag." {:offset offset :limit limit})))
  (let [tag
        (bit-and (.get buffer (int (checked-index offset))) 0xff)
    ...
    (case tag
      ...
      3
      [(bb-get-long buffer next) (+ next 8)]

      4
      [(bb-get-double buffer next) (+ next 8)]

      5
      (decode-string-at buffer next limit)

      6
      (let [ns-length
            (bb-get-int buffer next)
            ...
            [name-str after-name]
            (decode-string-at buffer after-ns limit)]

        [(if ns-str (keyword ns-str name-str) (keyword name-str)) after-name])

      7
      (let [n (bb-get-int buffer next)]
        (when (neg? n)
      ...
      (throw (ex-info "Unsupported CSR artifact key tag." {:tag tag :offset offset})))))

(defn- decode-key-section
  [^ByteBuffer buffer offsets-section bytes-section expected-count]
  (let [offsets-offset
        ...
        byte-count
        (:byte-length bytes-section)

        out
        (object-array expected-count)]
      ...
      (let [start (bb-get-long buffer (+ offsets-offset (* idx 8)))
        ...
        (when (or (neg? start) (< end start) (> end byte-count))
          (throw (ex-info "Invalid CSR artifact key offsets."
                          {:index idx :start start :end end :byte-count byte-count})))
        (let [[value next-pos] (decode-key-at buffer absolute-start absolute-end)]
          (when-not (= next-pos absolute-end)
            (throw (ex-info "CSR artifact key decoder did not consume the full key."
                            {:index idx :next-position next-pos :end absolute-end})))
          (aset out idx value))))
    out))

(defn- id-map
  ^HashMap [^objects keys]
  (let [m (HashMap.)]
  ...
  (loop [pos position]
    (when (.hasRemaining buffer)
      (let [n (.write channel buffer pos)]
        (when (neg? n)
          (throw (ex-info "Unexpected EOF while writing CSR artifact." {:position pos})))
        (recur (+ pos n))))))

(defn- section
  [name type count offset byte-length]
  {:id (section-ids name)
   :name name
...
(defn- required-section
  [sections name]
  (or (get sections (section-ids name))
      (throw (ex-info "CSR artifact is missing a required section." {:section name}))))
...
(defn- section-plan
  ...
  (let [row-count
        ...
        row-keys
        (mapv #(p/csr-row-key-at source %) (range row-count))

        col-keys
        (mapv #(p/csr-col-key-at source %) (range col-count))

        row-key-section
        (encode-key-section row-keys)

        col-key-section
        (encode-key-section col-keys)

        metadata-bytes
        (utf8-bytes (metadata-json row-count col-count entry-count block-dim))

        raw
        [{:name :row-ptrs :type :int32 :count (inc row-count) :byte-length (* 4 (inc row-count))}
         {:name :col-ids :type :int32 :count entry-count :byte-length (* 4 entry-count)}
         {:name :payload-values
          :type :float64
          :count (* entry-count block-dim)
          :byte-length (* 8 entry-count block-dim)}
         {:name :row-key-offsets
          :type :int64
          :count (inc row-count)
          :byte-length (* 8 (inc row-count))}
         {:name :row-key-bytes
          :type :bytes
          :count (alength ^bytes (:bytes row-key-section))
          :byte-length (alength ^bytes (:bytes row-key-section))}
         {:name :col-key-offsets
          :type :int64
          :count (inc col-count)
          :byte-length (* 8 (inc col-count))}
         {:name :col-key-bytes
          :type :bytes
          :count (alength ^bytes (:bytes col-key-section))
          :byte-length (alength ^bytes (:bytes col-key-section))}
         {:name :metadata
          :type :utf8-json
          :count (alength metadata-bytes)
          :byte-length (alength metadata-bytes)}]

        data-start
        (align8 (+ header-size (* section-entry-size (count raw))))]
    ...
    (loop [sections
      ...
      (if (empty? remaining)
        {:row-count row-count
         ...
         :file-size offset}
        (let [{:keys [name type count byte-length]}
              (first remaining)
...
(defn- header-buffer
  [{:keys [row-count col-count entry-count block-dim sections]}]
  (let [buffer
        ^ByteBuffer (buffer-order! (ByteBuffer/allocate header-size))
...
(defn- section-table-buffer
  [sections]
  (let [buffer ^ByteBuffer
               (buffer-order! (ByteBuffer/allocate (* section-entry-size (count sections))))]
...
(defn- write-int-section!
  [^FileChannel channel section values]
...
(defn- write-long-array-section!
  [^FileChannel channel section ^longs values]
  (let [buffer ^ByteBuffer (buffer-order! (ByteBuffer/allocate (int (:byte-length section))))]
...
(defn- write-byte-section!
  [^FileChannel channel section ^bytes values]
  (write-all! channel (ByteBuffer/wrap values) (:offset section)))

(defn- write-payload-section!
  [^FileChannel channel source section]
  (let [block-dim
        (p/csr-block-dim source)
        ...
        buffer
        ^ByteBuffer (buffer-order! (ByteBuffer/allocate (* 8 block-dim)))]
...
(defn- write-artifact-sections!
  [^FileChannel channel source plan section-by-name]
  (let [row-count
        (:row-count plan)
    ...
    (write-int-section!
      ...
      (concat (map (fn [row-id]
                   ...
                   (range row-count))
              [(if (zero? row-count) 0 (second (p/csr-row-span source (dec row-count))))]))
    ...
    (write-byte-section! channel (:row-key-bytes section-by-name) (:bytes (:row-key-section plan)))
    (write-long-array-section! channel
                               (:col-key-offsets section-by-name)
                               (:offsets (:col-key-section plan)))
    (write-byte-section! channel (:col-key-bytes section-by-name) (:bytes (:col-key-section plan)))
    (write-byte-section! channel (:metadata section-by-name) (:metadata-bytes plan))))
...
(defn- validate-section!
  [file-size {:keys [id name type-id offset byte-length count]}]
  (when-not (contains? section-names id)
    (throw (ex-info "CSR artifact contains an unknown section id." {:section-id id})))
  (when-not (contains? type-names type-id)
    (throw (ex-info "CSR artifact contains an unknown primitive type id."
                    {:section name :type-id type-id})))
  (when (or (neg? offset) (neg? byte-length) (> (+ offset byte-length) file-size))
    (throw (ex-info "CSR artifact section extends outside the file."
                    {:section name :offset offset :byte-length byte-length :file-size file-size})))
  (when-not (zero? (mod offset 8))
    (throw (ex-info "CSR artifact section is not 8-byte aligned." {:section name :offset offset})))
  (when (neg? count)
    (throw (ex-info "CSR artifact section has a negative count." {:section name :count count})))
...
(defn- read-section-table
  [^ByteBuffer buffer file-size section-count section-table-offset]
  (when-not (= section-table-offset header-size)
    (throw (ex-info "Unsupported CSR artifact section table offset."
                    {:section-table-offset section-table-offset :expected header-size})))
  (when (neg? section-count)
    (throw (ex-info "CSR artifact has a negative section count." {:section-count section-count})))
...
(defn- validate-required-sections!
  [{:keys [row-count col-count entry-count block-dim sections]}]
  (let [row-ptrs
        (required-section sections :row-ptrs)

        col-ids
        (required-section sections :col-ids)

        payload-values
        (required-section sections :payload-values)

        row-key-offsets
        (required-section sections :row-key-offsets)

        row-key-bytes
        (required-section sections :row-key-bytes)

        col-key-offsets
        (required-section sections :col-key-offsets)

        col-key-bytes
        (required-section sections :col-key-bytes)

        metadata
        (required-section sections :metadata)]

    (doseq [[section expected-type] [[row-ptrs :int32] [col-ids :int32] [payload-values :float64]
                                     [row-key-offsets :int64] [row-key-bytes :bytes]
                                     [col-key-offsets :int64] [col-key-bytes :bytes]
                                     [metadata :utf8-json]]]
      (when-not (= expected-type (:type section))
        (throw (ex-info
                 "CSR artifact section has the wrong primitive type."
                 {:section (:name section) :expected expected-type :actual (:type section)}))))
    (doseq [[section expected-count] [[row-ptrs (inc row-count)] [col-ids entry-count]
      ...
      (when-not (= expected-count (:count section))
        (throw (ex-info
                 "CSR artifact section has the wrong count."
                 {:section (:name section) :expected expected-count :actual (:count section)}))))
    {:row-ptrs row-ptrs
     :col-ids col-ids
...
(defn- validate-csr-arrays!
  [^ByteBuffer buffer {:keys [row-count col-count entry-count]} required]
  (let [row-ptrs
        (:row-ptrs required)
        ...
        (let [ptr (bb-get-int buffer (+ row-ptrs-offset (* row-id 4)))]
          (when (or (neg? ptr) (> ptr entry-count) (< ptr prev))
            (throw (ex-info "CSR artifact row pointers are invalid."
                            {:row-id row-id :previous prev :actual ptr :entry-count entry-count})))
    ...
    (let [last-ptr (bb-get-int buffer (+ row-ptrs-offset (* row-count 4)))]
      (when-not (= entry-count last-ptr)
        (throw (ex-info "CSR artifact final row pointer must equal entry count."
                        {:final-row-ptr last-ptr :entry-count entry-count}))))
...
(deftype MmapCSRSource [path ^MappedByteBuffer mapped sectionTable ^long rowCount ^long colCount
                        ^long entryCount ^long payloadDim ^objects idToRow ^objects idToCol
                        ^Map rowToId ^Map colToId ^long rowPtrsOffset ^long colIdsOffset
                        ^long payloadValuesOffset rangeIndex prefixFn]
  p/CSRSource
    ...
    (csr-row-key-at [_ row-id] (sparse/key-of idToRow (ensure-index! :row-id row-id rowCount)))
    (csr-col-key-at [_ col-id] (sparse/key-of idToCol (ensure-index! :col-id col-id colCount)))
    ...
    (csr-copy-block! [_ entry-id dst dst-off]
      (let [dst
            ...
            entry-id
            (ensure-index! :entry-id entry-id entryCount)

            dst-off
            (ensure-copy-target! dst dst-off payloadDim)

            start
            (+ payloadValuesOffset (* entry-id payloadDim 8))]
    ...
    (csr-resolve-ranges [_ selection]
      (cond (nil? selection) [(row-range 0 rowCount)]
        ...
        (let [file-size (.size channel)
          ...
          (let [artifact (parse-artifact mapped file-size)]
            (select-keys artifact
                         [:version :flags :row-count :col-count :entry-count :block-dim
                          :sections])))))))

(defn mmap-csr-source? [source] (instance? MmapCSRSource source))

(defn with-mmap-range-index
  [source prefix-fn range-index]
...
(defn mmap-prefix-fn [source] (.-prefixFn ^MmapCSRSource source))
