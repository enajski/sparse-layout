...
(deftype RowMap [ds rowKey ^long rowId mode
  ...
  (valAt [_ col-key not-found]
    (let [cid (sparse/id-of colToId col-key)]
      (if (neg? cid)
        not-found
        (let [pid (sparse/payload-id-by-ids rowId cid
                                            csrRowPtrs csrColIds
                                            cscColPtrs cscRowIds cscPayloadIds)]
          (if (= -1 pid)
            not-found
            (case mode
              :value (sparse/payload-value payloadKind payloadDim payloadValues payloadPtrs pid)
              :view  (sparse/payload-view  payloadKind payloadDim payloadValues payloadPtrs pid)))))))

  clojure.lang.Associative
  (containsKey [_ col-key]
    (let [cid (sparse/id-of colToId col-key)]
      (and (not (neg? cid))
           (not= -1 (sparse/payload-id-by-ids rowId cid
                                              csrRowPtrs csrColIds
                                              cscColPtrs cscRowIds cscPayloadIds)))))
      ...
      (let [row-id  (int rowId)
        ...
        (seq (map (fn [i]
                    (MapEntry. (sparse/key-of id->col (aget col-ids i))
                               (case mode
                                 :value (sparse/payload-value pk pd pv pp i)
                                 :view  (sparse/payload-view  pk pd pv pp i))))
                  (range start end))))
...
(deftype ColumnMap [ds colKey ^long colId mode
  ...
  (valAt [_ row-key not-found]
    (let [rid (sparse/id-of rowToId row-key)]
      (if (neg? rid)
        not-found
        (let [pid (sparse/payload-id-by-ids rid colId
                                            csrRowPtrs csrColIds
                                            cscColPtrs cscRowIds cscPayloadIds)]
          (if (= -1 pid)
            not-found
  ...
  (containsKey [_ row-key]
    (let [rid (sparse/id-of rowToId row-key)]
      (and (not (neg? rid))
           (not= -1 (sparse/payload-id-by-ids rid colId
                                              csrRowPtrs csrColIds
                                              cscColPtrs cscRowIds cscPayloadIds)))))
      ...
      (let [col-id      (int colId)
        ...
        (seq (map (fn [i]
                    (MapEntry. (sparse/key-of id->row (aget row-ids i))
                               (case mode
                                 :value (sparse/payload-value pk pd pv pp (aget payload-ids i))
                                 :view  (sparse/payload-view  pk pd pv pp (aget payload-ids i)))))
                  (range start end))))
...
(deftype DatasetMap [ds mode
  ...
  (valAt [_ row-key not-found]
    (let [rid (sparse/id-of rowToId row-key)]
      (if (neg? rid)
        not-found
        (RowMap. ds row-key rid mode
                 colToId idToCol csrRowPtrs csrColIds
                 cscColPtrs cscRowIds cscPayloadIds
                 payloadKind payloadDim payloadValues payloadPtrs))))

  clojure.lang.Associative
  (containsKey [_ row-key]
    (not (neg? (sparse/id-of rowToId row-key))))
  (entryAt [this row-key]
    (let [value (.valAt this row-key missing)]
      (when-not (identical? missing value)
        (MapEntry. row-key value))))
  ...
  clojure.lang.Seqable
  (seq [_]
    (let [rows ^objects idToRow
          n    (alength rows)]
      (seq (map (fn [row-id]
                  (let [rk (aget rows row-id)]
                    (MapEntry. rk
                               (RowMap. ds rk row-id mode
                                        colToId idToCol csrRowPtrs csrColIds
                                        cscColPtrs cscRowIds cscPayloadIds
                                        payloadKind payloadDim payloadValues payloadPtrs))))
                (range n)))))
...
(defn- row-map* [ds m row-key row-id mode]
  (RowMap. ds row-key row-id mode
           (:col->id m) (:id->col m)
           (:csr-row-ptrs m) (:csr-col-ids m)
           (:csc-col-ptrs m) (:csc-row-ids m) (:csc-payload-ids m)
           (:payload-kind m) (:payload-dim m)
           (:payload-values m) (:payload-ptrs m)))

(defn- col-map* [ds m col-key col-id mode]
  (ColumnMap. ds col-key col-id mode
              (:row->id m) (:id->row m)
              (:csr-row-ptrs m) (:csr-col-ids m)
              (:csc-col-ptrs m) (:csc-row-ids m) (:csc-payload-ids m)
              (:payload-kind m) (:payload-dim m)
...
(defn row-map
  ...
  [ds row-key]
  (let [m   (sparse/sparse-internals ds)
        rid (sparse/id-of (:row->id m) row-key)]
    (when-not (neg? rid)
      (row-map* ds m row-key rid :value))))
...
(defn row-view-map
  ...
  (let [m   (sparse/sparse-internals ds)
        rid (sparse/id-of (:row->id m) row-key)]
    (when-not (neg? rid)
      (row-map* ds m row-key rid :view))))

(defn col-map
  "Returns a read-only column map facade: row key -> copied payload value."
  [ds col-key]
  (let [m   (sparse/sparse-internals ds)
        cid (sparse/id-of (:col->id m) col-key)]
    (when-not (neg? cid)
      (col-map* ds m col-key cid :value))))
