...
(def ^:private allowed-payload-kinds #{:double :long :object :fixed-double-block :var-double-block})

(def ^:private allowed-duplicate-policies #{:last :sum :merge :error})

(def ^:private double-array-class (Class/forName "[D"))

(defn- numeric-seqable? [value] (and (not (nil? value)) (not (map? value)) (seqable? value)))
...
(deftype IntBuffer [^:unsynchronized-mutable ^ints data ^:unsynchronized-mutable ^int size]
  BufferAccess
    (buf-data [_] data)
...
(deftype DoubleBuffer [^:unsynchronized-mutable ^doubles data ^:unsynchronized-mutable ^int size]
  BufferAccess
    (buf-data [_] data)
    (buf-set-data! [_ new-data] (set! data new-data))
    (buf-size [_] size)
...
(deftype LongBuffer [^:unsynchronized-mutable ^longs data ^:unsynchronized-mutable ^int size]
  BufferAccess
    (buf-data [_] data)
    (buf-set-data! [_ new-data] (set! data new-data))
    (buf-size [_] size)
...
(deftype ObjectBuffer [^:unsynchronized-mutable ^objects data ^:unsynchronized-mutable ^int size]
  BufferAccess
    (buf-data [_] data)
    (buf-set-data! [_ new-data] (set! data new-data))
    (buf-size [_] size)
...
(deftype DoubleBlockView [^doubles values ^int offset ^int length]
  Object
    (toString [_] (str "#<DoubleBlockView offset=" offset " length=" length ">")))
...
(defn- grow-capacity
  ^long [^long current ^long needed]
  (loop [capacity (max 1 current)]
    (if (>= capacity needed) capacity (recur (* 2 capacity)))))

(defn- ib-ensure!
  [^IntBuffer buffer ^long needed]
...
(defn- db-ensure!
  [^DoubleBuffer buffer ^long needed]
...
(defn- ob-ensure!
  [^ObjectBuffer buffer ^long needed]
...
(defn- db-add!
  [^DoubleBuffer buffer ^double value]
  (let [size (buf-size buffer)]
...
(defn- ib-get ^long [^IntBuffer buffer ^long i] (aget ^ints (buf-data buffer) (int i)))

(defn- db-get ^double [^DoubleBuffer buffer ^long i] (aget ^doubles (buf-data buffer) (int i)))

(defn- lb-get ^long [^LongBuffer buffer ^long i] (aget ^longs (buf-data buffer) (int i)))

(defn- ob-get [^ObjectBuffer buffer ^long i] (aget ^objects (buf-data buffer) (int i)))

(defn- ib-size ^long [^IntBuffer buffer] (buf-size buffer))

(defn- db-size ^long [^DoubleBuffer buffer] (buf-size buffer))

(defn- ib->array
  ^ints [^IntBuffer buffer]
  (let [size
        (buf-size buffer)
...
(defn- ex [message data] (ex-info message data))

(defn- normalize-payload-spec
  [payload]
  (let [payload
        (cond (nil? payload) {:kind :double}
              (keyword? payload) {:kind payload}
              (map? payload) payload
              :else (throw (ex "Payload must be a keyword or map." {:payload payload})))

        kind
        (:kind payload)]

    (when-not (contains? allowed-payload-kinds kind)
      (throw (ex "Unsupported payload kind." {:kind kind :allowed allowed-payload-kinds})))
    (when (= kind :fixed-double-block)
      (when-not (pos-int? (:dim payload))
        (throw (ex "Fixed double blocks require a positive :dim." {:payload payload}))))
    payload))

(defn- normalize-layout
  [layout]
  (let [layout
        (if (map? layout) layout (throw (ex "Layout must be a map." {:layout layout})))

        duplicate-policy
        (or (:duplicate-policy layout) :last)

        indices
        (set (or (:indices layout) #{:csr}))

        retain-coo?
        (if (contains? layout :retain-coo?) (boolean (:retain-coo? layout)) true)]

    (when-not (contains? allowed-duplicate-policies duplicate-policy)
      (throw (ex "Unsupported duplicate policy."
                 {:duplicate-policy duplicate-policy :allowed allowed-duplicate-policies})))
    (assoc layout
...
(defn- double-array-length
  ^long [value]
  (cond (instance? double-array-class value) (alength ^doubles value)
        (numeric-seqable? value) (count value)
        :else -1))

(defn- copy-double-values!
  [dest offset value expected label]
  (let [dest
        ^doubles dest
      ...
      (if (numeric-seqable? value)
        (do
          ...
          (loop [i
            ...
            (when xs (aset dest (int (+ offset i)) (double (first xs))) (recur (inc i) (next xs)))))
        (throw (ex "Payload must be a double array or sequence of numbers."
                   {:value value :label label}))))))

(defn- double-array-value
  ^doubles [value expected label]
  (let [actual (double-array-length value)]
    ...
    (when (and expected (not= (long expected) actual))
      (throw (ex "Unexpected double block length."
                 {:expected expected :actual actual :label label})))
    (let [out (double-array actual)]
      (copy-double-values! out 0 value actual label)
      out)))

(defn- append-fixed-double-block!
  [^DoubleBuffer values ^long dim value]
...
(defn- append-var-double-block!
  [^DoubleBuffer values value]
  (cond (instance? double-array-class value)
        (let [value
              ^doubles value
        ...
        (numeric-seqable? value) (doseq [x value]
                                   (db-add! values (double x)))
        :else (throw (ex "Variable double blocks require a double array or sequence."
                         {:value value}))))
...
(deftype DoublePayloadBuffer [^DoubleBuffer values]
  PayloadBuffer
    (-append-payload! [_ value] (db-add! values (double value)))
    (-payload-at [_ edge-id] (db-get values edge-id))
    (-payload-count [_] (db-size values))
    (-payload-kind [_] :double)
    (-payload-dim [_] 1)
    (-gather-storage [_ ids m]
      (let [ids
        ...
        {:payload-values out :payload-ptrs nil})))

(deftype LongPayloadBuffer [^LongBuffer values]
  PayloadBuffer
    (-append-payload! [_ value] (lb-add! values (long value)))
    (-payload-at [_ edge-id] (lb-get values edge-id))
    (-payload-count [_] (buf-size values))
    (-payload-kind [_] :long)
    (-payload-dim [_] 1)
    (-gather-storage [_ ids m]
      (let [ids
...
(deftype ObjectPayloadBuffer [^ObjectBuffer values]
  PayloadBuffer
    (-append-payload! [_ value] (ob-add! values value))
    (-payload-at [_ edge-id] (ob-get values edge-id))
    (-payload-count [_] (buf-size values))
    (-payload-kind [_] :object)
    (-payload-dim [_] 1)
    (-gather-storage [_ ids m]
      (let [ids
            ^ints ids
...
(deftype FixedDoubleBlockPayloadBuffer [^long dim ^DoubleBuffer values ^IntBuffer count]
  PayloadBuffer
    (-append-payload! [_ value] (append-fixed-double-block! values dim value) (ib-add! count 1))
    (-payload-at [_ edge-id]
      (let [out
        ...
        out))
    (-payload-count [_] (ib-size count))
    (-payload-kind [_] :fixed-double-block)
    (-payload-dim [_] dim)
    (-gather-storage [_ ids m]
      (let [ids
            ^ints ids
        ...
        (dotimes [i m]
          (System/arraycopy data (int (* (aget ids i) dim)) out (int (* i dim)) (int dim)))
        {:payload-values out :payload-ptrs nil})))
  ...
  PayloadBuffer
    ...
    (-payload-at [_ edge-id]
      (let [start
        ...
        out))
    (-payload-count [_] (dec (ib-size ptrs)))
    (-payload-kind [_] :var-double-block)
    (-payload-dim [_] -1)
    (-gather-storage [_ ids m]
      (let [ids
            ^ints ids
        ...
        (dotimes [i m]
          (let [id (aget ids i)
            ...
            (System/arraycopy src-vals start (buf-data out-values) (int dest-start) length)
            (buf-set-size! out-values (+ dest-start length))
            (aset out-ptrs (inc i) (int (db-size out-values)))))
        {:payload-values (db->array out-values) :payload-ptrs out-ptrs})))

(defn- payload-buffer
  [payload]
  (case (:kind payload)
    :double
    (DoublePayloadBuffer. (double-buffer))

    :long
    (LongPayloadBuffer. (long-buffer))

    :object
    (ObjectPayloadBuffer. (object-buffer))

    :fixed-double-block
    (FixedDoubleBlockPayloadBuffer. (long (:dim payload)) (double-buffer) (int-buffer))
...
(deftype SparseBuilder [layout ^HashMap rowToId ^ArrayList idToRow ^HashMap colToId
                        ^ArrayList idToCol ^IntBuffer rows ^IntBuffer cols payloadBuffer])
...
(defn- intern-id!
  ^long [^HashMap forward ^ArrayList reverse key]
...
(defn append-entry!
  ...
  (let [row-id
        (intern-id! (.-rowToId builder) (.-idToRow builder) row-key)

        col-id
        (intern-id! (.-colToId builder) (.-idToCol builder) col-key)]
...
(defn- sum-double-values
  ^doubles [left right expected]
  (let [left
        (double-array-value left expected :sum-left)

        right
        (double-array-value right expected :sum-right)
...
(defn- sum-payload
  ...
  (case (:kind payload)
    ...
    :fixed-double-block
    (sum-double-values left right (:dim payload))

    :var-double-block
    (let [left-length
          (double-array-length left)

          right-length
          (double-array-length right)]
    ...
    :object
    (throw (ex "Cannot :sum object payloads." {:payload payload}))))

(defn- combine-duplicate
  [layout left right row-id col-id]
  (case (:duplicate-policy layout)
    :last
    right

    :sum
    (sum-payload (:payload layout) left right)

    :merge
    (if-let [merge-fn (:merge-fn layout)]
      (merge-fn left right)
      (throw (ex "Duplicate policy :merge requires :merge-fn." {:row-id row-id :col-id col-id})))

    :error
    (throw (ex "Duplicate sparse coordinate." {:row-id row-id :col-id col-id}))))
    ...
    (loop [pos
      ...
      (if (= pos n)
        (do
          ...
          {:rows (ib->array out-rows) :cols (ib->array out-cols) :payload-ids (ib->array out-ids)})
        (let [edge-id
              (aget order pos)
          ...
          (if (and have-current? (= current-row row) (= current-col col))
            (if error?
              (throw (ex "Duplicate sparse coordinate." {:row-id row :col-id col}))
    ...
    (loop [pos
      ...
      (if (= pos n)
        (do (when have-current?
            ...
            {:rows (ib->array out-rows) :cols (ib->array out-cols) :payloads out-payloads})
        (let [edge-id
              (aget order pos)
              ...
              payload
              (-payload-at payload-buffer edge-id)]
...
(defn- coalesce-builder
  [^SparseBuilder builder]
  (let [rows
        (ib->array (.-rows builder))

        cols
        (ib->array (.-cols builder))

        payload-buffer
        (.-payloadBuffer builder)

        layout
        (.-layout builder)

        policy
        (:duplicate-policy layout)

        row-count
        (.size ^ArrayList (.-idToRow builder))

        col-count
        (.size ^ArrayList (.-idToCol builder))
    ...
    (when-not (= n (-payload-count payload-buffer))
      (throw (ex "COO coordinate and payload counts diverged."
                 {:coordinates n :payloads (-payload-count payload-buffer)})))
...
(defn- compile-payload-storage
  [payload ^ArrayList payloads]
  (let [n (.size payloads)]
    (case (:kind payload)
      :double
      (let [values (double-array n)]
        ...
        {:payload-values values :payload-ptrs nil})
      ...
      (let [ptrs (int-array (inc n))
        ...
        {:payload-values (db->array values) :payload-ptrs ptrs}))))

(defn- build-csr
  [indices ^ints edge-rows ^ints edge-cols row-count nnz]
  (when (contains? indices :csr)
    (let [row-ptrs
      ...
      (dotimes [row row-count]
        (aset row-ptrs (inc row) (+ (aget row-ptrs row) (aget row-ptrs (inc row)))))
      (dotimes [payload-id nnz]
        (aset col-ids payload-id (aget edge-cols payload-id)))
      {:csr-row-ptrs row-ptrs :csr-col-ids col-ids})))
...
(defn- build-csc
  ...
  (when (contains? indices :csc)
    (let [col-count
      ...
      {:csc-col-ptrs col-ptrs :csc-row-ids row-ids :csc-payload-ids payload-ids})))

(defn- frozen-map ^Map [^HashMap source] (Collections/unmodifiableMap (HashMap. source)))

(defn- frozen-base
  [^SparseBuilder builder layout payload ^ints edge-rows ^ints edge-cols]
...
(defn freeze-builder!
  ...
  (let [layout
        ...
        csr
        (or (build-csr (:indices layout) edge-rows edge-cols row-count nnz) {})

        csc
        (or (build-csc (:indices layout) edge-rows edge-cols col-count nnz) {})]

    (merge (frozen-base builder layout payload edge-rows edge-cols) payload-storage csr csc)))

(defn id-of
  ^long [^Map id-map key]
  (let [value (.get id-map key)]
    (if (nil? value) -1 (int value))))

(defn key-of
  [^objects keys ^long id]
  (when (and (<= 0 id) (< id (alength keys))) (aget keys (int id))))
...
(defn- find-payload-in-csr
  ...
  (if (nil? csr-row-ptrs)
    ...
    (let [csr-col-ids
      ...
      (loop [lo
             (aget row-ptrs (int row-id))

             hi
             (dec (aget row-ptrs (inc (int row-id))))]
        ...
        (if (> lo hi)
          ...
          (let [mid
                ...
                edge-col
                (aget csr-col-ids mid)]

            (cond (= edge-col col-id) mid
...
(defn- find-payload-in-csc
  ...
  (if (nil? csc-col-ptrs)
    ...
    (let [csc-row-ids
      ...
      (loop [lo
             (aget col-ptrs (int col-id))

             hi
             (dec (aget col-ptrs (inc (int col-id))))]
        ...
        (if (> lo hi)
          ...
          (let [mid
                ...
                edge-row
                (aget csc-row-ids mid)]

            (cond (= edge-row row-id) (aget csc-payload-ids mid)
                  (< edge-row row-id) (recur (inc mid) hi)
                  :else (recur lo (dec mid)))))))))

(defn payload-id-by-ids
  [row-id col-id csr-row-ptrs csr-col-ids csc-col-ptrs csc-row-ids csc-payload-ids]
  (if (and (<= 0 row-id) (<= 0 col-id))
    (do (when (and (nil? csr-row-ptrs) (nil? csc-col-ptrs))
          (throw (ex "Neither CSR nor CSC index was compiled." {:required #{:csr :csc}})))
...
(defn payload-value
  [payload-kind payload-dim payload-values payload-ptrs payload-id]
  (case payload-kind
    :double
    (let [values
          ^doubles payload-values
    ...
    (let [values
          ...
          out
          (double-array payload-dim)

          start
          (* payload-id payload-dim)]
    ...
    (let [values
          ...
          start
          (aget payload-ptrs (int payload-id))

          end
          (aget payload-ptrs (inc (int payload-id)))
...
(defn block-view-array ^doubles [^DoubleBlockView view] (.-values view))

(defn block-view-offset ^long [^DoubleBlockView view] (.-offset view))

(defn block-view-length ^long [^DoubleBlockView view] (.-length view))

(defn block-view-value
  ^double [^DoubleBlockView view ^long i]
  (let [offset
        (.-offset view)
    ...
    (when-not (and (<= 0 i) (< i length))
      (throw (ex "Block view index out of bounds." {:index i :length length})))
    (aget ^doubles (.-values view) (int (+ offset i)))))

(defn block-view->array
  ^doubles [^DoubleBlockView view]
  (let [length
        (.-length view)
...
(defn block-view->vec
  ...
  (let [values
    ...
    (loop [i
      ...
      (if (= i length) (persistent! out) (recur (inc i) (conj! out (aget values (+ offset i))))))))

(defn block-by-ids
  [row-id col-id csr-row-ptrs csr-col-ids csc-col-ptrs csc-row-ids csc-payload-ids payload-kind
   payload-dim payload-values payload-ptrs]
...
(defn row-blocks-by-id
  [row-id id-to-col csr-col-ids csr-row-ptrs payload-kind payload-dim payload-values payload-ptrs]
  (cond (neg? row-id) []
        (nil? csr-row-ptrs) (throw (ex "CSR index was not compiled for row traversal."
                                       {:required :csr}))
        :else (let [id-to-col
                    ^objects id-to-col
                    ...
                    start
                    (aget csr-row-ptrs (int row-id))

                    end
                    (aget csr-row-ptrs (inc (int row-id)))]
...
(defn col-blocks-by-id
  [col-id id-to-row csc-row-ids csc-col-ptrs csc-payload-ids payload-kind payload-dim payload-values
   payload-ptrs]
  (cond (neg? col-id) []
        (nil? csc-col-ptrs) (throw (ex "CSC index was not compiled for column traversal."
                                       {:required :csc}))
        :else (let [id-to-row
                    ^objects id-to-row
                    ...
                    start
                    (aget csc-col-ptrs (int col-id))

                    end
                    (aget csc-col-ptrs (inc (int col-id)))]
...
(defmacro defsparse
  ...
  (let [base-name
        ...
        layout-var
        (symbol (str base-name "-layout"))

        make-builder-name
        (symbol (str "make-" base-name "-builder"))

        ingest-name
        (symbol (str base-name "-ingest!"))

        freeze-name
        (symbol (str base-name "-freeze!"))

        compile-name
        (symbol (str base-name "-compile"))

        row-id-name
        (symbol (str base-name "-row-id"))

        col-id-name
        (symbol (str base-name "-col-id"))

        row-name
        (symbol (str base-name "-row"))

        col-name
        (symbol (str base-name "-col"))

        block-name
        (symbol (str base-name "-block"))

        row-views-name
        (symbol (str base-name "-row-views"))

        col-views-name
        (symbol (str base-name "-col-views"))

        block-view-name
        (symbol (str base-name "-block-view"))

        marker-method-name
        (symbol (str base-name "-dataset?"))

        record-sym
        (gensym "record")

        ds-sym
        (with-meta (gensym "ds") {:tag type-name})
        ...
        row-expr
        (path-form record-sym row-path)

        cols-expr
        (path-form record-sym cols-path)]

    `(do
       (def ~layout-var ~layout)
       (defprotocol ~marker-protocol-name
         (~marker-method-name [~'this]))
       (deftype ~type-name [^java.util.Map ~'rowToId ^objects ~'idToRow ^java.util.Map ~'colToId
                            ^objects ~'idToCol ^ints ~'edgeRows ^ints ~'edgeCols ^ints ~'csrRowPtrs
                            ^ints ~'csrColIds ^ints ~'cscColPtrs ^ints ~'cscRowIds
                            ^ints ~'cscPayloadIds ~'payloadKind ^long ~'payloadDim ~'payloadValues
                            ^ints ~'payloadPtrs]
         sparse-layout.core/SparseDataset
           (~'row-id [~'this row-key#] (sparse-layout.core/id-of ~'rowToId row-key#))
           (~'col-id [~'this col-key#] (sparse-layout.core/id-of ~'colToId col-key#))
           (~'row-blocks
             [~'this row-key#]
         ...
         Object
           (~'toString
             ...
             (str "#<"
                  ...
                  (alength ~'idToCol)
                  " nnz="
                  (cond (some? ~'csrColIds) (alength ~'csrColIds)
