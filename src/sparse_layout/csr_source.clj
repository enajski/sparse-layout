(ns sparse-layout.csr-source
  (:require [sparse-layout.core :as sparse]
            ...
            [sparse-layout.csr-source.protocols :as p
             :refer [CSRSource MutableSparseDelta MergedCSRSource]])
...
(defn- build-range-index [source prefix-fn]
  (let [acc (HashMap.)
        row-count (p/csr-row-count source)]
...
(defn- normalize-block-dim [block-dim]
  (if (and (integer? block-dim) (pos? (long block-dim)))
    (long block-dim)
    (throw (ex-info "Delta block dimension must be a positive integer."
                    {:block-dim block-dim}))))

(defn- ensure-block-length! [block expected]
...
(defn- copy-block [block expected]
  (let [out (cond
...
(defn- emit-main!
  ...
  (let [col-id (p/csr-entry-col-id source entry-id)
        col-key (p/csr-col-key-at source col-id)]
...
(defn scan-merged-row-by-id!
  ...
  (let [row-key (p/csr-row-key-at source row-id)
        [start end] (p/csr-row-span source row-id)
    ...
    (loop [main-entry start
      ...
      (cond
        ...
        :else
        (let [main-col-id (p/csr-entry-col-id source main-entry)
              main-col-key (p/csr-col-key-at source main-col-id)
              delta-entry (nth delta-entries delta-idx)
              delta-col-key (:col-key delta-entry)
              delta-col-id (p/csr-col-id source delta-col-key)
